...
Image::Image(const Image& origClass){
  cout << "Copy Constructor Called" << endl;

  //clear();
  width = origClass.width;
  height = origClass.height;
  filename = origClass.filename;
  image = new Pixel*[width];
  for (unsigned int col=0; col < width; ++col) {
      image[col] = new Pixel[height];
      *image[col] = *(origClass.image[col]);
  }

  for (unsigned int row = 0; row < height; ++row) {
      for (unsigned int col = 0; col < width; ++col) {
        this->image[col][row] = origClass.image[col][row];
      }
  }
}
Image& Image::operator=(const Image& origClass){
