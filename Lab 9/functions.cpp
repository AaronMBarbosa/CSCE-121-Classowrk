...
void printMineMatrix (struct minePosition ** matrix, int nRows, int nCols, int nMines)
{
  ...
  for (int i = 0; i < nRows; i ++){
	...
	if (matrix[i][j].neighborMines == 0) 
	  cout << 0; 
	else if (matrix[i][j].neighborMines == -1)
	  cout << "*"; 
	else
...
{ 
  ...
  if (mineMatrix[row][col].neighborMines != 0) {
    ...
    if (countNeighborMines(mineMatrix, nRows, nCols, row, col) == 0 && mineMatrix[row][col].open == true && mineMatrix[row][col].neighborMines != 0)
    {
        mineMatrix[row][col].neighborMines = 0;
		if (row + 1 < nRows) {
